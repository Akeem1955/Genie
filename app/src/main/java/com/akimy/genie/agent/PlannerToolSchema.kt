...
class PlannerToolSchema : ToolSet {
    ...
    @Tool(description = "Finish the current goal and provide the final user-facing summary.")
    fun finish_task(@ToolParam(description = "A brief summary of what Genie accomplished or why it stopped.") summary: String) =
        declared()

    private fun declared(): Map<String, String> = mapOf("status" to "schema_only")
}

fun geniePlannerToolProviders(): List<ToolProvider> = listOf(tool(PlannerToolSchema()))
